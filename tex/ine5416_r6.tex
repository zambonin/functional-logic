...
\begin{itemize}
    ...
    \item A orientação a objetos não é tão natural em Haskell como em outras linguagens. Embora esta exista, é comum abordar problemas de outras maneiras quando utiliza-se uma linguagem funcional. Não existe, por exemplo, uma superclasse universal como \texttt{Object}, e o encapsulamento deve ser feito através do gerenciamento de módulos.
    \item Listas e tuplas são estruturas fundamentais para manipulação de grupos de valores. A restrição mais importante é que todos os elementos da lista devem ser do mesmo tipo. Existe um operador de adição de elementos à lista que pode ser utilizado da seguinte maneira: \texttt{<elemento>:[]}, onde a lista é delimitada por colchetes. De modo contrário, tuplas aceitam elementos de tipos diferentes, mas são imutáveis no seu tamanho, ou seja, o operador \texttt{:} não deve funcionar. $n$-uplas são tuplas de tamanho $n$, ou seja, que contêm $n$ elementos.
\end{itemize}
